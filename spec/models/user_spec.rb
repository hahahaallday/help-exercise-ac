...
RSpec.describe User, type: :model do
  ...
  describe "#words_count" do
    before do
      ...
      # other_user = User.create!( :email => "ooxx@example.org", :password => "12345678")
      # m1 = Message.create!( :user => other_user, :title => "This is a test title", :content =>"This is content")
      # Comment.create!( :user => other_user, :message => m1, :content => "this is comment content1" )

      m2 = Message.create!( :user => @user ,:title => "This is a test title", :content =>"This is content")
      Comment.create!( :user => @user, :message => m2 ,:content =>"This is comment content2" )
      Comment.create!( :user => @user, :message => m2 ,:content => "this is comment content3" )
    end

    it "加總該使用者的所有 Mesasge 和 Comment 的總字數" do
      # TODO: 測試 words_count 方法
      expect( @user.words_count ).to eq(16)
    end
  end
